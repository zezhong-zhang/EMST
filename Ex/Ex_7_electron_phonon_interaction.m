...
still_output_multislice = il_multem(system_conf, input_multem);
toc;
...
for i=1:length(output_multislice.data)
    ...
    title(strcat('Frozen phonon Total intensity-Thick = ', num2str(output_multislice.thick(i))));
    axis image;
    colormap gray;
    subplot(1,2,2)
    imagesc(still_output_multislice.data(i).m2psi_tot);
    title(strcat('Still atom Total intensity-Thick = ', num2str(still_output_multislice.thick(i))));
    axis image;
    colormap gray;
    pause(0.25);
end