...
input_multem.interaction_model = 3;              % eESIM_Multislice = 1, eESIM_Phase_Object = 2, eESIM_Weak_Phase_Object = 3
weak_output_multislice = il_multem(system_conf, input_multem);
toc;
...
for i=1:length(output_multislice.data)
    ...
    title(strcat('Multislice Total intensity-Thick = ', num2str(output_multislice.thick(i))));
    axis image;
    colormap gray;
    subplot(1,2,2)
    imagesc(weak_output_multislice.data(i).m2psi_tot);
    title(strcat('Weak phase Total intensity-Thick = ', num2str(weak_output_multislice.thick(i))));
    axis image;
    colormap gray;
    pause(0.25);
end