...
input_multem.scanning_x0 = 3*3*a;
input_multem.scanning_y0 = 3*sqrt(3)*a;
input_multem.scanning_xe = 4*3*a;
input_multem.scanning_ye = 4*sqrt(3)*a;
...
input_multem.detector.cir(2).inner_ang = 60;  % Inner angle(mrad)
...
for j=1:ndet
    subplot(1, ndet, j);
    imagesc(output_multislice.data.image_tot(j).image);
    axis image;
    colormap gray;
end
